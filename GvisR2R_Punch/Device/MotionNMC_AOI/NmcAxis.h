...
#define ONE_MINUTE		60000		 // Wait for max 10 minute.
...
// Construction
	...
	CNmcAxis(CNmcDevice* pParent=NULL);

// Attributes
public: 
	int EscapeHwPosLimit();

	void CheckStatusAndClear();
	BOOL WaitStandStill();
	double GetActVel();
	//2021 08 12 lgh add
	BOOL GetOriginStatus();
	double CalSpeedProfile(double fLength, double &fVel, double &fAcc, double &fDec);
