...
CNmcDevice::CNmcDevice(CWnd* pParent)
{
	...
	m_pParamCtrl = &(((CMotionNew*)m_pParent)->m_ParamCtrl);
	m_pParamMotion = ((CMotionNew*)m_pParent)->m_pParamMotion;
	m_pParamAxis = ((CMotionNew*)m_pParent)->m_pParamAxis;
	m_pParamMotor = ((CMotionNew*)m_pParent)->m_pParamMotor;
...
double CNmcDevice::CalSpeedProfile(int nAxisID, double fLength, double &fVel, double &fAcc, double &fDec)
{
	...
	double fAccPeriod = ((CMotionNew*)m_pParent)->m_pParamAxis[nAxisID].fAccPeriod;			// m_fAccPeriod[nAxisID];
	double fAccLength;
	fAcc = ((CMotionNew*)m_pParent)->m_pParamAxis[nAxisID].fAcc;							// m_fAcc[nAxisID];

	if (fAcc <= 0.0 || fAcc > ((CMotionNew*)m_pParent)->m_pParamAxis[nAxisID].fMaxAccel)	// m_fMaxAccel[nAxisID])
	...
	{
		fVel = fAcc*fAccTime;
		if (fAccTime < ((CMotionNew*)m_pParent)->m_pParamAxis[nAxisID].fMinJerkTime)	// m_fMinJerkTime[nAxisID])
			fAccTime = ((CMotionNew*)m_pParent)->m_pParamAxis[nAxisID].fMinJerkTime;	// m_fMinJerkTime[nAxisID];
		fAcc = fVel / fAccTime;
		fDec = fAcc;
	}
