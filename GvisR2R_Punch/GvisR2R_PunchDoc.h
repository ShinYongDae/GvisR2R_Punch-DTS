...
public:
	...
	CReelMap *m_pReelMapInOuterUp, *m_pReelMapInOuterDn;	// 내/외 층 Merge한 릴맵
...
//	int LoadPCR(int nSerial, BOOL bFromShare = FALSE);		// return : 2(Failed), 1(정상), -1(Align Error, 노광불량), -2(Lot End)
	...
	BOOL SetInnerInfo(int nSerial);
	BOOL SetOuterInfo(int nSerial);
	BOOL SetDefaultInfo(int nSerial);
